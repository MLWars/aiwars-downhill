//! The MCP server for the Downhill referee. A faithful copy of the chess warden's
//! `mcp.rs`, typed to `Match<Downhill>`: a SINGLE bearer-gated `/mcp` endpoint
...
//! line (e.g. `tuck:straight`) instead of a UCI move.
...
use crate::downhill::Downhill;
use aiwars_mcp_warden::game::Match;

/// Shared, mutex-guarded match state shared across all agents' requests.
pub type SharedMatch = Arc<Mutex<Match<Downhill>>>;

/// `make_move` tool arguments.
#[derive(Debug, Deserialize, JsonSchema)]
pub struct MakeMoveArgs {
    /// The line to ride, one of your legal moves (e.g. "tuck:straight",
    /// "carve:turn", "jump:shortcut").
    pub mv: String,
...
impl RefereeHandler {
    #[tool(
        description = "Get the current run state: your progress/crashes, the rival, the slope report, status, winner, and your legal lines.",
    ...
    #[tool(
        description = "List the lines you may ride now (tuck:straight / carve:turn / jump:shortcut) and whether it is your turn, with the current ply.",
        annotations(read_only_hint = true, open_world_hint = false)
    )]
    ...
    #[tool(
        description = "Ride a line. Pass the ply you saw as expected_ply. Tuck is fastest but reckless — trees, ice and a buried branch can wipe you out; carve is safe and steady; jump skips ahead but risks a bad landing. Returns the new state on success; an error (no change) if illegal, out of turn, or stale.",
    ...
    #[tool(
        description = "Pull off the course (forfeit). Your rival is awarded the win.",
        annotations(read_only_hint = false, destructive_hint = false, open_world_hint = false)
    )]
...
impl ServerHandler for RefereeHandler {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "AIWars Downhill Daredevils referee. Call get_state / legal_moves to observe, make_move \
             to ride a line (with the ply you saw), resign to forfeit. Tuck the fall line for raw \
             speed but risk trees, ice and a buried branch; carve clean and steady; jump the \
             shortcut and pray the landing. First sled to the finish wins. Your seat is identified \
             by your bearer token; you cannot act as your rival.",
        )
    }
}
