//! Downhill Daredevils — a turn-based alpine sled race refereed exactly like chess.
//!
...
//!   - `tuck:straight` — the fall line: fast but reckless (trees/ice/avalanche can
//!     wipe you out → crash, lose ground)
...
//! seed ⇒ identical line options (deterministic / replayable), mirroring how
...
impl Sled {
    fn new(lane: u8) -> Self {
        Self { prog: 0, leg: 0, lane, crashes: 0, last_crash: false, done: false }
    }
}
...
impl Game for Downhill {
    ...
    fn apply(&mut self, agent: usize, mv: &str) -> Result<(), MatchError> {
        ...
        let leg = self.sleds[agent].leg;
        let lines = self.lines(leg);
        let idx = lines
            .iter()
            .position(|l| l.name == mv)
            .ok_or_else(|| MatchError::IllegalMove(format!("'{mv}' is not a line here")))?;
        ...
        let roll = frac(
            self.seed,
            leg,
            53 + (agent as u32 + 1) * 9 + self.ply * 3,
        );
        let crashed = (roll * 10000.0) < crash_bp as f64;

        let s = &mut self.sleds[agent];
    ...
    fn state(&self, handles: &[String]) -> Value {
        ...
        let leading = leader.map(h).map(Value::String).unwrap_or(Value::Null);
        let winner = self
            .winner_idx
        ...
        // Slope report for the sled about to move (avalanche/ice/groomed).
...
mod tests {
    ...
    fn handles() -> Vec<String> {
        vec!["comet".to_string(), "cruise".to_string()]
    }
    ...
    fn rejects_wrong_player_count() {
        for n in [1usize, 3] {
            ...
            match Match::<Downhill>::new(hs, &json!({})) {
    ...
    fn illegal_and_out_of_turn_rejected_without_change() {
        ...
        // wrong agent
        assert_eq!(m.make_move(1, "tuck:straight", 0).unwrap_err(), MatchError::NotYourTurn);
        // bogus line
        assert!(matches!(
            m.make_move(0, "skate:flat", 0).unwrap_err(),
    ...
    fn stale_ply_rejected() {
        let mut m = Match::<Downhill>::new(handles(), &json!({ "seed": 7 })).unwrap();
        m.start();
        assert_eq!(m.make_move(0, "tuck:straight", 9).unwrap_err(), MatchError::StalePly);
    }

    #[test]
    fn a_full_run_resolves_to_winner_or_draw() {
        // Both sleds always take the first legal line: a decisive result must
        // emerge (someone crosses the finish or the leg cap is hit) with a
        // concrete winner or a draw.
        let mut m = Match::<Downhill>::new(handles(), &json!({ "seed": 7 })).unwrap();
        m.start();
        let mut guard = 0;
    ...
    fn resign_awards_opponent() {
        let mut m = Match::<Downhill>::new(handles(), &json!({ "seed": 3 })).unwrap();
        m.start();
        let st = m.resign(0);
    ...
    fn same_seed_same_lines() {
        let a = Match::<Downhill>::new(handles(), &json!({ "seed": 42 })).unwrap();
        let b = Match::<Downhill>::new(handles(), &json!({ "seed": 42 })).unwrap();
    ...
    fn same_seed_same_full_game() {
        ...
        let play = |seed: u64| -> Value {
            let mut m = Match::<Downhill>::new(handles(), &json!({ "seed": seed })).unwrap();
            m.start();
            let mut guard = 0;
            while !m.is_resolved() && guard < 64 {
                let seat = m.state_json()["to_move_idx"].as_u64().unwrap() as usize;
