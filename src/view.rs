...
//! the chess warden's `view.rs`, typed to `Match<Downhill>`. Serves `/state.json`
...
pub struct ViewState {
    pub game: Arc<Mutex<Match<Downhill>>>,
}

/// `GET /state.json` — the live run JSON the SPA polls. A pure read.
...
mod tests {
    ...
    fn match_fixture() -> Arc<Mutex<Match<Downhill>>> {
        let handles = vec!["comet".to_string(), "cruise".to_string()];
        let m = Match::<Downhill>::new(handles, &serde_json::json!({ "seed": 7 })).unwrap();
        Arc::new(Mutex::new(m))
    }
    ...
    #[tokio::test]
    async fn state_json_is_a_readable_run() {
        let resp = router()
        ...
        assert_eq!(v["game"], "downhill");
        assert!(v["sleds"].is_array());
    }
